...
There are a lot of options and commands for this tool that can be found eg. on the [OffSec](https://www.offsec.com/metasploit-unleashed/msfconsole-commands/){:target="_blank" rel="noopener"} website. In fact, you can even find there a great free full course on that topic - [Metasploit Unleashed](https://www.offsec.com/metasploit-unleashed/){:target="_blank" rel="noopener"}. As for now, we will try to determine the Samba version on the host. Let's check if there is a module that can help me with that...

...
With the contents of those files, we get hashes of all passwords so we can then attempt to decrypt them in order to gain access to the system as a regular user or with root privileges in the future. Additionally, we can try to remove any system logs to cover our tracks. This path to resolution was easy and you can compare it to [Lame](https://app.hackthebox.com/machines/1){:target="_blank" rel="noopener"} - the easiest machine on [Hack The Box](https://hackthebox.com){:target="_blank" rel="noopener"}.

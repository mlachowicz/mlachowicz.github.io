...
Scanning shows that we have plenty of open ports and outdated services - potential attack vectors. 

The next tool we're going to use is [Nikto](https://github.com/sullo/nikto){:target="_blank" rel="noopener"}. It's a web server scanner that checks a website for many possible security problems, such as dangerous files, misconfigured services, and vulnerable scripts.
